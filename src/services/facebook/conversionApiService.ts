...
interface ConversionApiEventData {
...
export class FacebookConversionApiService {
   ...
   */
  async sendLeadEvent(params: SendEventParams): Promise<void> {
    ...
    const numericLeadId = parseInt(
      crypto.createHash('md5').update(leadId).digest('hex').substring(0, 15),
      16
    );

    const userData: ConversionApiEventData['user_data'] = {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Facebook Conversion API error: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
