...
export class AggregateController {
   ...
   */
  async getAggregatedReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      let actualResults;
      if (isFullYear) {
        actualResults = await this.actualService.getAggregatedActualsForAllUsers(
          String(startDateStr),
          String(endDateStr)
        );
      } else {
      ...
      const targetResults = await this.targetService.getAggregatedTargetsForAllUsers(
        String(startDateStr),
        String(endDateStr),
