import WeeklyTarget from './models/target.model.js';
export class TargetRepository {
    ...
    async debugAllTargetsForUser(userId) {
        const allTargets = await this.model.find({ userId }).sort({ startDate: 1 });
        return allTargets;
    }
    async getTargetsByDateRange(startDate, endDate, userId) {
    ...
    async findTargetByStartDate(userId, startDate) {
     ...
     */
    async getMonthlyTargetsByWeeks(userId, weeksInMonth) {
        const { DateUtils } = await import('../../../utils/date.utils.js');
        const results = await Promise.all(weeksInMonth.map(async (week) => {
     ...
     */
    async findTargetsByQuery(query) {
