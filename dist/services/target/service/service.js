...
export class TargetService {
     ...
     */
    _aggregateTargets(targets, queryType, userId, startDate, endDate) {
        if (targets.length === 0) {
            return {
                userId: userId || "",
     ...
     */
    _aggregateTargetsForAllUsers(targets, queryType, startDate, endDate) {
        if (targets.length === 0) {
            return {
                userId: "ALL_USERS",
                startDate: startDate || new Date().toISOString(),
        ...
        const decodedTargets = targets.map(target => {
    ...
    validateQueryTypeChange(existingQueryType, newQueryType) {
        // Allowed changes
    ...
    isDateInPastOrCurrent(targetDate, queryType) {
        const now = new Date();
    ...
    async upsertWeeklyTarget(userId, startDate, endDate, data, queryType) {
        try {
            ...
            if (existingTarget) {
                ...
                const filteredData = Object.fromEntries(Object.entries(data).filter(([key, value]) => value !== null && value !== undefined));
                const updatedData = {
    ...
    async _upsertMonthlyTarget(userId, startDate, endDate, data, queryType) {
        try {
            ...
            const monthlyUpsertPromises = weeksInMonth.map((week, index) => {
                return this.upsertWeeklyTarget(userId, week.weekStart, week.weekEnd, monthlyProratedData, queryType);
            });
    ...
    async _upsertYearlyTarget(userId, startDate, endDate, data, queryType) {
        try {
            ...
            for (let month = 0; month < 12; month++) {
                ...
                if (monthStart <= currentMonth) {
                    ...
                    const existingTargets = await this.getWeeklyTargetsInRange(userId, monthStart.toISOString().split('T')[0], monthEnd.toISOString().split('T')[0], queryType);
                    if (existingTargets.length > 0) {
                        ...
                        const weeksInMonth = DateUtils.getMonthWeeks(monthStart.toISOString().split('T')[0], monthEnd.toISOString().split('T')[0]);
                        for (const week of weeksInMonth) {
                            const zeroFilledTarget = await this.upsertWeeklyTarget(userId, week.weekStart, week.weekEnd, {
                                ...data,
                ...
                if (monthStart <= currentMonth) {
                    ...
                    if (weeksInMonth.length > 0) {
                        ...
                        for (const week of weeksInMonth) {
                            ...
                            // Only create this target if it belongs to the current month being processed
                            if (targetMonth === month) {
                                const weeklyTarget = await this.upsertWeeklyTarget(userId, week.weekStart, week.weekEnd, monthlyData, queryType);
    ...
    async upsertTargetByPeriod(userId, startDate, endDate, queryType, data) {
        try {
            switch (queryType) {
                case "weekly":
                    return this.upsertWeeklyTarget(userId, startDate, endDate, data, queryType);
                case "monthly":
                    return this._upsertMonthlyTarget(userId, startDate, endDate, data, queryType);
                case "yearly":
                    return this._upsertYearlyTarget(userId, startDate, endDate, data, queryType);
                default:
    ...
    async getWeeklyTarget(userId, startDate) {
        const weekInfo = DateUtils.getWeekDetails(startDate);
    ...
    async getWeeklyTargetsInRange(userId, startDate, endDate, queryType = "any") {
        ...
        const weeklyTargets = await Promise.all(weeksInRange.map(week => this.getWeeklyTarget(userId, week.weekStart)));
        return weeklyTargets;
    }
    async getAggregatedMonthlyTarget(userId, startDate, endDate, queryType) {
        const weeksInMonth = DateUtils.getMonthWeeks(startDate, endDate);
        if (weeksInMonth.length === 0) {
            return [];
        }
    ...
    async getYearlyTargetsOrganizedByMonths(userId, startDate, endDate, queryType) {
        const year = new Date(startDate).getFullYear();
        const monthlyTargets = [];
        ...
        for (let month = 0; month < 12; month++) {
            ...
            const monthResults = await this.getAllWeeksOrganizedByMonths(userId, monthStartStr, monthEndStr, "monthly");
            // Add the first (and only) month's data to our yearly results
            monthlyTargets.push(monthResults.length > 0 ? monthResults[0] : []);
        }
        return monthlyTargets;
    }
    async getAllWeeksOrganizedByMonths(userId, startDate, endDate, queryType) {
        ...
        const weeksInRange = DateUtils.getMonthWeeks(startDate, endDate);
        const allWeeklyTargets = await Promise.all(weeksInRange.map(week => this.getWeeklyTarget(userId, week.weekStart)));
     ...
     */
    async getAggregatedTargetsForAllUsers(startDateStr, endDateStr, queryType) {
        switch (queryType) {
            case "weekly":
     ...
     */
    async getWeeklyTargetForAllUsers(startDate) {
        const weekInfo = DateUtils.getWeekDetails(startDate);
     ...
     * Uses decode-average-reencode logic
     */
    async getAggregatedMonthlyTargetForAllUsers(startDate, endDate) {
        const weeksInMonth = DateUtils.getMonthWeeks(startDate, endDate);
        if (weeksInMonth.length === 0) {
            return [];
        }
        // For each week, get all users' targets and aggregate
        const weeklyTargets = await Promise.all(weeksInMonth.map(async (week) => {
     ...
     * Uses decode-average-reencode logic
     */
    async getAggregatedYearlyTargetForAllUsers(startDate, endDate) {
        const year = new Date(startDate).getFullYear();
        const results = [];
        for (let month = 0; month < 12; month++) {
            const monthStart = new Date(year, month, 1);
