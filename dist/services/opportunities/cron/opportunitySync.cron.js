        ...
        try {
            ...
            for (const opp of opportunities) {
                ...
                if (!lower.has('facebook lead'))
                    continue;
                // Check which target tags are present (excluding 'facebook lead' for special case)
            ...
            if (uniqueJobWonContactIds.length > 0) {
                ...
                for (const contactId of uniqueJobWonContactIds) {
                    try {
                        const resp = await client.get(`/contacts/${encodeURIComponent(contactId)}`, {
                            headers: {
                        ...
                        if (Array.isArray(customFields)) {
                            const field = customFields.find((f) => f?.id === CUSTOM_FIELD_ID_TO_SUM);
                        ...
                        logger.warn('Failed to fetch contact for job_won sum', { contactId, error: e?.message || String(e) });
                    }
                }
            }
            // eslint-disable-next-line no-console
            ...
            // Estimates Ran: count unique opportunities that have 'facebook lead' tag AND ANY of these tags: job_won, job_lost, appt_completed, appt_completed_unresponsive, color_consultation_booked
            ...
            try {
                ...
                const savedActual = await actualService.upsertActualWeekly(userId, startDate, endDate, {
                    leads,
                    estimatesRan,
                    estimatesSet,
                    sales: jobBooked,
