 ...
 */
function parseCustomField2Date(dateStr) {
    try {
 ...
 */
function isDateInWeekRange(date, startDateStr, endDateStr) {
    if (!date)
        return false;
...
async function getContactDate(contactId, locationId, decryptedToken, tags, customFieldId2, retry = { retries: 3, baseDelayMs: 1000 }) {
    try {
        ...
        const resp = await withRetry(async () => {
            try {
                return await clientHttp.get(`/contacts/${encodeURIComponent(contactId)}`, {
                    headers: {
    ...
    async runOnce(type) {
        ...
        try {
            ...
            for (const client of clients) {
                ...
                let opportunities = [];
                try {
                    ghlResponse = await withRetry(() => opportunitySyncService.fetchOpportunities(locationId, decryptedToken), retry);
                ...
                const counts = Object.fromEntries(TARGET_TAGS.map((t) => [t, 0]));
                const opportunitiesToProcess = [];
                for (const opp of opportunities) {
                    if (!opp?.pipelineId || !opp?.contactId)
                        continue;
                    const collected = [];
                    const contactTags = opp?.contact?.tags;
                    if (Array.isArray(contactTags))
                        collected.push(...contactTags);
                    const relations = opp?.relations;
                    if (Array.isArray(relations)) {
                        for (const rel of relations) {
                            if (Array.isArray(rel?.tags))
                                collected.push(...rel.tags);
                        }
                    }
                    if (collected.length === 0)
                        continue;
                    const lower = new Set(collected.map((t) => String(t).toLowerCase()));
                ...
                const contactDateCache = new Map();
                ...
                const tagAnalysisLog = {
                    ...
                    tagCounts: counts,
                    tagDetails: Object.fromEntries(Object.entries(tagDetails).map(([tag, data]) => [
                        tag,
                ...
                if (uniqueJobWonContactIds.length > 0 && customFieldId) {
                    ...
                    for (const contactId of uniqueJobWonContactIds) {
                        try {
                            const resp = await withRetry(() => clientHttp.get(`/contacts/${encodeURIComponent(contactId)}`, {
                                headers: {
                                    Authorization: `Bearer ${decryptedToken}`,
                                    Version: '2021-07-28',
                                },
                ...
                let savedActual;
                try {
                    savedActual = await withRetry(() => actualService.upsertActualWeekly(userId, startDate, endDate, uploadData), retry);
                    ...
                    const apiResponse = {
                        ...
                        createdAt: savedActualObj.createdAt || savedActualObj.created_at,
                        updatedAt: savedActualObj.updatedAt || savedActualObj.updated_at,
                    };
