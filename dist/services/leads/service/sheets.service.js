...
import { parseEstimateSetValue, validateSheetHeaders, getRequiredSheetHeaders, validateSheetUrl, extractSheetId, extractGid, mapSheetRowToLead } from "../utils/sheet.util.js";
 ...
 */
const normalizeLeadsFromSheet = (leads, existingLeads) => {
    return leads.map((lead) => {
        const isEstimateSet = lead.status === "estimate_set";
        const hasUnqualifiedReason = lead.unqualifiedLeadReason && lead.unqualifiedLeadReason.trim() !== "";
...
export class SheetsService {
     ...
     */
    async fetchLeadsFromSheet(sheetUrl, clientId) {
        if (!validateSheetUrl(sheetUrl))
            throw new Error("Invalid Google Sheet URL");
        const sheetId = extractSheetId(sheetUrl);
        if (!sheetId)
            throw new Error("Failed to extract sheet ID from URL");
        // Extract GID (sub-sheet ID) from URL if present
        let targetGid = extractGid(sheetUrl);
        ...
        let data = [];
        let allHeaders = [];
        let targetSheetName = "Unknown";
        let availableSubSheets = [];
        try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            try {
                ...
                const status = isEstimateSet ? 'estimate_set' : 'unqualified';
     ...
     */
    extractConversionRateInsights(leads) {
     ...
     */
    async processSheetData(sheetUrl, clientId) {
        // 1. Fetch and parse leads from sheet
