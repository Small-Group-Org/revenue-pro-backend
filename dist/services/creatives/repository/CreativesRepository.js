...
export class CreativesRepository {
     ...
     */
    async getCreativesByIds(creativeIds) {
     ...
     */
    async getCreativesByAdAccount(adAccountId, limit = 100) {
    ...
    async deleteCreative(creativeId) {
        const result = await CreativeModel.updateOne({ creativeId }, { $set: { isDeleted: true, deletedAt: new Date() } });
        return result.modifiedCount > 0;
    }
    /**
     * Get total count of creatives
     */
    async getCreativesCount(adAccountId) {
