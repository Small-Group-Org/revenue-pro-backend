...
export class FbWeeklyAnalyticsRepository {
     ...
     */
    async saveWeeklyAnalytics(data) {
        const filter = {
        ...
        return await FbWeeklyAnalyticsModel.findOneAndUpdate(filter, { $set: update }, { upsert: true, new: true }).exec();
    }
    /**
     * Bulk save multiple weekly analytics records
     * Uses bulkWrite for efficiency
     */
    async bulkSaveWeeklyAnalytics(dataArray) {
     ...
     */
    async getAnalyticsByDateRange(clientId, startDate, endDate) {
     ...
     */
    async getAnalyticsByAdAndDateRange(clientId, adId, startDate, endDate) {
        return await FbWeeklyAnalyticsModel.find({
            clientId,
            ad_id: adId,
            weekStartDate: { $lte: endDate },
     ...
     */
    async getAnalyticsByClientId(clientId) {
        return await FbWeeklyAnalyticsModel.find(this.addSoftDeleteFilter({ clientId })).exec();
    }
    /**
     * Get analytics by campaign
     */
    async getAnalyticsByCampaign(clientId, campaignId) {
        return await FbWeeklyAnalyticsModel.find(this.addSoftDeleteFilter({ clientId, campaign_id: campaignId })).exec();
    }
    /**
     * Soft delete analytics record
     */
    async deleteAnalytics(id) {
        return await FbWeeklyAnalyticsModel.findByIdAndUpdate(id, { $set: { isDeleted: true, deletedAt: new Date() } }, { new: true }).exec();
    }
    /**
     * Soft delete all analytics for a client within a date range
     */
    async deleteAnalyticsByDateRange(clientId, startDate, endDate) {
     ...
     */
    async getAggregatedByCampaign(clientId, startDate, endDate) {
