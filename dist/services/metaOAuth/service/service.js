...
export class MetaOAuthService {
    ...
    async exchangeCodeForToken(code) {
        try {
            ...
            const response = await axios.post(`https://graph.facebook.com/${config.META_API_VERSION}/oauth/access_token`, null, {
                params: {
                    client_id: config.META_CLIENT_ID,
                    client_secret: config.META_CLIENT_SECRET,
                    redirect_uri: config.META_REDIRECT_URI,
    ...
    async getLongLivedToken(shortLivedToken) {
        try {
            ...
            const response = await axios.get(`https://graph.facebook.com/${config.META_API_VERSION}/oauth/access_token`, {
                params: {
                    grant_type: 'fb_exchange_token',
                    client_id: config.META_CLIENT_ID,
                    client_secret: config.META_CLIENT_SECRET,
     ...
     */
    async storeMetaToken(userId, tokenData) {
        try {
            if (!userId) {
     ...
     */
    async completeOAuthFlow(userId, code) {
        try {
