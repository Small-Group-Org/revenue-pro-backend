...
class GhlClientController {
    constructor() {
        this.ghlClientService = ghlClientService;
        /**
         ...
         */
        this.createGhlClient = async (req, res) => {
            try {
                ...
                if (!locationId || !ghlApiToken || !queryValue || !revenueProClientId) {
                    ...
                    utils.sendErrorResponse(res, new CustomError(ErrorCode.BAD_REQUEST, "Missing required fields: locationId, ghlApiToken, queryValue, and revenueProClientId are required"));
                    return;
                }
                const ghlClient = await this.ghlClientService.createGhlClient(locationId, ghlApiToken, queryValue, revenueProClientId, 'active', queryValue2);
         ...
         */
        this.getAllGhlClients = async (req, res) => {
            try {
         ...
         */
        this.getGhlClientById = async (req, res) => {
            try {
         ...
         */
        this.updateGhlClient = async (req, res) => {
            try {
                const { locationId } = req.params;
                ...
                if (!ghlApiToken && !queryValue && queryValue2 === undefined && !revenueProClientId && !status) {
                    ...
                    utils.sendErrorResponse(res, new CustomError(ErrorCode.BAD_REQUEST, "At least one field must be provided for update: ghlApiToken, queryValue, queryValue2, revenueProClientId, or status"));
                    return;
                }
                // Validate status enum if provided
                if (status && !['active', 'inactive', 'deleted'].includes(status)) {
                    logger.warn('[GHL Client Update] Invalid status value', { locationId, status });
                    utils.sendErrorResponse(res, new CustomError(ErrorCode.BAD_REQUEST, "Status must be one of: active, inactive, deleted"));
                    return;
                }
                const updatedClient = await this.ghlClientService.updateGhlClientByLocationId(locationId, {
                    ghlApiToken,
                    queryValue,
                    queryValue2,
                    revenueProClientId,
