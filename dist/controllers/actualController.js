...
export class ActualController {
     ...
     */
    async getActualAndTargetReport(req, res) {
        try {
            ...
            if (isFullYear) {
                actualResults = await this.actualService.getActualYearlyMonthlyAggregate(String(userId), start.getFullYear());
            }
            else {
                actualResults = await this.actualService.getActualsByDateRange(String(userId), String(startDateStr), String(endDateStr));
            }
            ...
            switch (queryType) {
                case "weekly":
                    const weeklyTarget = await targetService.getWeeklyTarget(String(userId), String(startDateStr));
                    targetResults = [weeklyTarget];
                    break;
                case "monthly":
                    targetResults = await targetService.getAggregatedMonthlyTarget(String(userId), String(startDateStr), String(endDateStr), "monthly");
                    break;
                case "yearly":
                    targetResults = await targetService.getAggregatedYearlyTarget(String(userId), String(startDateStr), String(endDateStr), "yearly");
                    break;
                default:
                    targetResults = [];
            }
    ...
    async upsertActual(req, res) {
        try {
            ...
            const upserted = await this.actualService.upsertActualWeekly(userId, startDate, endDate, actualData);
    ...
    async getActuals(req, res) {
        try {
            ...
            let results;
            if (isFullYear) {
                results = await this.actualService.getActualYearlyMonthlyAggregate(String(userId), start.getFullYear());
            }
            else {
                results = await this.actualService.getActualsByDateRange(String(userId), String(startDateStr), String(endDateStr));
            }
     ...
     */
    async updateWeeklyReporting(req, res) {
        try {
            ...
            if (revenue !== undefined)
                updateData.revenue = Number(revenue);
            if (leads !== undefined)
                updateData.leads = Number(leads);
            if (estimateRan !== undefined)
                updateData.estimatesRan = Number(estimateRan);
            if (estimateSet !== undefined)
                updateData.estimatesSet = Number(estimateSet);
            if (jobBooked !== undefined)
                updateData.sales = Number(jobBooked);
            ...
            const updated = await this.actualService.updateWeeklyReporting(userId, startDate, updateData);
            res.status(200).json({
                success: true,
