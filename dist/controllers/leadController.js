...
export class LeadController {
     ...
     */
    async processLeadScoresAndCRs(req, res) {
        try {
            let clientIds = [];
            if (req.query.clientId) {
                if (req.query.clientId === "all") {
                    ...
                    if (!clientIds || clientIds.length === 0) {
                        utils.sendErrorResponse(res, "No clients with associated lead data were found.");
                        return;
                    }
                }
                else {
                    ...
                    if (!clientExists) {
                        utils.sendErrorResponse(res, `No user found for clientId: ${clientId}.`);
                        return;
                    }
                    // Step 2: Validate client has lead data
                    const hasLeadData = await this.service.hasLeadData(clientId);
                    if (!hasLeadData) {
                        utils.sendErrorResponse(res, `No leads found for clientId: ${clientId}.`);
                        return;
                    }
                    clientIds = [clientId];
                }
                ...
                utils.sendErrorResponse(res, `Required clientId parameter is missing`);
                return;
            }
            const results = [];
            for (const clientId of clientIds) {
                try {
                    const result = await this.service.processLeadScoresAndCRsByClientId(clientId);
                    results.push({
                        clientId,
                        processing: {
                        ...
                        summary: {
                            processingSuccessRate: result.totalProcessedLeads > 0
            ...
            console.error("Error in processLeadScoresAndCRs endpoint:", error);
     ...
     */
    async updateLeadByEmail(req, res) {
     ...
     */
    async getLeadsPaginated(req, res) {
        try {
            ...
            const result = await this.service.getLeadsPaginated(clientId, startDate, endDate, { page, limit, sortBy, sortOrder }, filters);
    ...
    async getLeadFiltersAndCounts(req, res) {
        try {
            ...
            const data = await this.service.fetchLeadFiltersAndCounts(clientId, startDate, endDate);
            utils.sendSuccessResponse(res, 200, {
                success: true,
                data,
            });
    ...
    async getAnalytics(req, res) {
        try {
            ...
            const analytics = await this.service.getLeadAnalytics(clientId, startDate, endDate, sort);
            res.json({
                success: true,
                data: analytics
            });
    ...
    async getAnalyticsTable(req, res) {
        try {
            const clientId = typeof req.query.clientId === "string" ? req.query.clientId : undefined;
            const startDate = typeof req.query.startDate === "string" ? req.query.startDate : undefined;
            const endDate = typeof req.query.endDate === "string" ? req.query.endDate : undefined;
            const adSetPage = req.query.adSetPage ? parseInt(req.query.adSetPage, 15) : 1;
            const adNamePage = req.query.adNamePage ? parseInt(req.query.adNamePage, 10) : 1;
            const adSetItemsPerPage = req.query.adSetItemsPerPage ? parseInt(req.query.adSetItemsPerPage, 10) : 15;
            const adNameItemsPerPage = req.query.adNameItemsPerPage ? parseInt(req.query.adNameItemsPerPage, 10) : 10;
            const adSetSortField = req.query.adSetSortField;
            const adSetSortOrder = req.query.adSetSortOrder;
            const adNameSortField = req.query.adNameSortField;
            const adNameSortOrder = req.query.adNameSortOrder;
            const showTopRanked = req.query.showTopRanked === 'true';
            const performanceData = await this.service.getPerformanceTables(clientId, startDate, endDate, adSetPage, adNamePage, adSetItemsPerPage, adNameItemsPerPage, {
                adSetSortField,
                adSetSortOrder,
                adNameSortField,
                adNameSortOrder,
    ...
    async deleteLead(req, res) {
        try {
            ...
            const message = deletedResult.deletedCount > 0
        ...
        try {
            ...
            utils.sendSuccessResponse(res, 200, {
                ...
                data: {
                    ...
                    summary: {
                        processingSuccessRate: `${((processingResult.validLeadsProcessed /
    ...
    async hubspotSubscription(req, res) {
        try {
    ...
    async syncClientActivity(req, res) {
        try {
